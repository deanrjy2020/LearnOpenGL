...
/*
todo, 从shadertoy的FS里面移植到CS里, 还有很多可以做的
- 目前同样的code, 在ThinkPad X1 intel gpu上, FPS个位数, 在Jetson AGX Xavier上 FPS 24.
- 提升perf, 如何优化
- 用鼠标移动camera, 前提是FPS不会太慢
- scene信息由CPU ssbo传过来, 而不是在cs里面写死
- build BVH, 传到CS里面

看vk cs rt bvh里面的实现, 不用看这个ogl的实现了.
*/
...
// quad texture size
const unsigned int QUAD_WIDTH = 800, QUAD_HEIGHT = 600;
const unsigned int wgSize = 16; // 16*16*1
...
int main(int argc, char* argv[])
{
    ...
    // spheres.push_back({ glm::vec3(0, 5, -1), 2, glm::vec3(0.8, 0.8, 0.0) });
    // spheres.push_back({ glm::vec3(0, 0, -1), 4, glm::vec3(0.7, 0.3, 0.3) });

    // GLuint sphereSSBO;
    // glGenBuffers(1, &sphereSSBO);
    // glBindBuffer(GL_SHADER_STORAGE_BUFFER, sphereSSBO);
    // glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(Sphere) * spheres.size(), spheres.data(), GL_STATIC_DRAW);
    // // int sphere_count = (int) spheres.size();
    // // glBufferSubData(GL_SHADER_STORAGE_BUFFER,           0,                     sizeof(int), &sphere_count);
    ...
    vector<ComputeShader> computeShaders = {
        ComputeShader("rt_in_one_weekend_gpu.cs"),
    };
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        computeShaders[shaderIdx].setFloat("time", currentFrame);
        glm::vec2 iRes = glm::vec2(QUAD_WIDTH, QUAD_HEIGHT);
        computeShaders[shaderIdx].setVec2("iResolution", iRes);
        //computeShaders[shaderIdx].setFloat("sphereCount", spheres.size());
        ...
        glDispatchCompute((unsigned int)(QUAD_WIDTH+wgSize-1)/wgSize, (unsigned int)(QUAD_HEIGHT+wgSize-1)/wgSize, 1);
