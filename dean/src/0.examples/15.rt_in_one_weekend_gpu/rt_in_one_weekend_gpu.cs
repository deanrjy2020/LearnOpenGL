...
uniform vec3 front;
uniform vec3 up;
uniform vec3 right; // pass from CPU, don't do cross prod in each shader
...
void Camera_init(out Camera camera, vec3 lookfrom, vec3 lookat, vec3 vup, float vfov, float aspect, float defocusAngle, float focusDist)
{
    camera.lensRadius = defocusAngle / 2.0;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    Camera camera;
    Camera_init(camera, lookfrom, lookat, vec3(0.0f, 1.0f, 0.0f), 20.0f, float(iResolution.x) / float(iResolution.y), defocusAngle, focusDist);
...
void main() {
    ...
    Ray ray = {
        position,                                            // origin
        normalize(front + coeff.x * right + coeff.y * up)    // direction.
    };
    ...
    // //pixel = hitScene1(ray, hit);

    // pixel = hitScene2(ray, hit);

    vec4 FragColor = vec4(pixel, 1.0);
    mainImage(FragColor, pixelCoord);

    imageStore(imgOutput, pixelCoord, FragColor);
}